/**
 * Scout APM demo app — Node 24 + Express (latest)
 *
...
captureAgent.start()
    ...
    .then(() => {
        const config = buildScoutConfiguration({
            name: process.env.SCOUT_NAME || "scout-node24-express-demo",
        ...
        app.get("/api/hello", (_req, res) => {
            res.json({ message: "Hello from the internal API!", runtime: "node24", framework: "express" });
        });
        ...
        const server = app.listen(PORT, () => {
            console.log(`Node ${process.version} + Express demo app listening on port ${PORT}`);
        });
