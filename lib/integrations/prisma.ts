...
// Shape of a span emitted by Prisma's query engine (Prisma 6+).
// Mirrors the EngineSpan type from @prisma/instrumentation-contract.
...
export class PrismaIntegration extends RequireIntegration {
    ...
    private getPrismaMajorVersion(prismaExport: any): number {
        try {
            const version: string = prismaExport?.Prisma?.prismaVersion?.client ?? "";
    ...
    // -------------------------------------------------------------------------
    private registerTracingHelper(majorVersion: number): void {
        const integration = this;
        const storage = new AsyncLocalStorage<{ sql: string | null; traceparent: string }>();
        ...
        const helper = {
            ...
            runInChildSpan(
                nameOrOpts: string | Record<string, any>,
                ...
                const sqlStore: { sql: string | null; traceparent: string } = { sql: null, traceparent };

                return integration.scout.instrument(ScoutSpanOperation.SQLQuery, async (done: () => void) => {
                    ...
                    try {
                        ...
                        if (span) {
                            await span.addContext(
                                ScoutContextName.DBStatement,
                                sqlStore.sql ?? "(unknown)",
                            );
                        }

                        done();
                        integration.logFn(
                            `[scout/integrations/prisma] operation completed`,
                            LogLevel.Trace,
                        );
                        return result;
                    } catch (err: any) {
        ...
        const value = { helper };
        (globalThis as any)[`V${majorVersion}_PRISMA_INSTRUMENTATION`] = value;
        (globalThis as any).PRISMA_INSTRUMENTATION = value;
    }
