...
export class Scout extends EventEmitter {
    ...
    public instrument(operation: string, cb: DoneCallback): Promise<any> {
        ...
        const doneFn = () => {
            console.log("this.asyncNamespace?", this.asyncNamespace);
            ...
            console.log("AFTER?", this.asyncNamespace);
            return span.stop();
        };
     ...
     * before someone tries to clear it. This can happen if some caller moves calls to another async context or if it's cleaned up suddenly
     */
