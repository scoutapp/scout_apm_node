...
test("Winston logger is successfully logged to", t => {
    ...
    Promise.resolve(tempfile())
        ...
        scout = new scout_1.Scout(types_1.buildScoutConfiguration({ allowShutdown: true, monitor: true }), { logFn });
    })
...
test("Scout inherits winston logger level", t => {
    ...
    Promise.resolve(tempfile())
        ...
        const logFn = types_1.buildWinstonLogFn(logger);
        // Build scout instance
        scoutConfig = types_1.buildScoutConfiguration({ allowShutdown: true, monitor: true });
