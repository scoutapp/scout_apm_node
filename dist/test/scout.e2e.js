...
test("Application metadata is built and sent", t => {
    ...
    const config = lib_1.buildScoutConfiguration({ allowShutdown: true, monitor: true }, {
        env: {
    ...
    const listener = (message) => {
        console.log("message with type:", message.type);
    ...
    // Set up listener on the agent
    scout.on(types_1.AgentEvent, listener);
    scout
