...
// NOTE: *all* other core-agent instances need to be killed for this test to work properly
test("agent reconnects to intermittently connected core-agent", { timeout: TestUtil.PG_TEST_TIMEOUT_MS * 1000 }, t => {
    ...
    const scout = new scout_1.Scout(config, { appMeta, logFn: types_1.consoleLogFn });
    ...
    const listener = (data) => {
        ...
        if (!agentKilled && !agentRestarted) {
            t.pass(`pre-disconnect request was seen [${data.request.id}]`);
        ...
        if (agentKilled && !agentRestarted) {
            ...
            t.fail(`Unexpected between-restart request was seen [${data.request.id}]`);
            throw new Error("Successful request was sent after agent disconnected");
        }
        // if it isn't the case that the agent was killed and restarted already, exit early
        if (!(agentKilled && agentRestarted)) {
            return;
        }
        scout.removeListener(types_1.ScoutEvent.RequestSent, listener);
        ...
        t.assert(successfulRequests[0].id !== data.request.id, "a new request was sent");
        t.pass(`request sent after disconnection & reconnection [${data.request.id}]`);
        TestUtil.shutdownScout(t, scout);
    };
    ...
    scout
        ...
        .then(() => agent.start())
        .then(() => t.comment("agent restarted"))
